...
class IglesiaRequest extends FormRequest
{
        ...
        return [
            'nombre' => [
                    ...
                    function($attribute, $value, $fail) {
                        // compara sin mayúsculas/minúsculas y sin acentos (Postgres unaccent)
                        $exists = DB::table('iglesias')
                            ->whereRaw("unaccent(lower(nombre)) = unaccent(lower(?))", [$value])
                            ->exists();

                        if ($exists) {
                            $fail('Ya existe una iglesia con este nombre.');
                        }
                    },
                ],
    ...
    {
        return [
             'nombre.unique' => 'Ya existe una iglesia con este nombre.',
