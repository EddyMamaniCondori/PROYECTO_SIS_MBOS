...
<body>
    ...
    @php
        // 1. Cálculos de Totales Globales basados en el detalle de semanas
        $sumDiezmo = $semanas->sum('diezmo_total');
        $sumOfrenda = $semanas->sum('ofrenda_total');
        $sumProTemplo = $semanas->sum('pro_templo_total');
        
        // Suma de todo lo recolectado
        $totalGeneralBruto = $sumDiezmo + $sumOfrenda + $sumProTemplo;

        // Distribución 40/60 de la ofrenda
        $ofrendaMision40 = $sumOfrenda * 0.40;
        $ofrendaLocal60 = $sumOfrenda * 0.60;

        // REMESA MBOS (Lo que se paga a la misión)
        $remesaMbosFinal = $sumDiezmo + $ofrendaMision40;

        // FONDO LOCAL (Lo que se queda en la iglesia)
        $fondoLocalFinal = $sumProTemplo + $ofrendaLocal60;
    @endphp
    ...
    <table class="resumen-container">
        <tr>
            <td class="card-resumen">
                <div class="card-header-blue" style="background-color: #475569;">Total Recolectado</div>
                <div class="card-body-amount">
                    Bs {{ number_format($totalGeneralBruto, 2) }}
                </div>
                <div class="card-footer-sub">Ingresos Brutos del Mes</div>
            </td>

            <td class="card-resumen">
                <div class="card-header-blue">Fondo Local (Iglesia)</div>
                <div class="card-body-amount">
                    Bs {{ number_format($fondoLocalFinal, 2) }}
                </div>
                <div class="card-footer-sub">Pro-Templo + 60% Ofrendas</div>
            </td>

            <td class="card-resumen card-mbos">
                <div class="card-header-blue">Remesa MBOS (Misión)</div>
                <div class="card-body-amount" style="color: #0d47a1; font-size: 22px;">
                    Bs {{ number_format($remesaMbosFinal, 2) }}
                </div>
    ...
    <div class="info-label" style="margin-left: 5px; margin-bottom: 5px;">Detalle Semanal de Ingresos</div>
    <table>
        <thead>
            <tr>
                <th>Sem</th>
        ...
        <tbody>
            ...
            @php $subTotalSemana = $sem->diezmo_total + $sem->ofrenda_total + $sem->pro_templo_total; @endphp
            <tr>
                <td>{{ $sem->nro_semana }}</td>
                <td>{{ number_format($sem->diezmo_total, 2) }}</td>
                <td>{{ number_format($sem->ofrenda_total, 2) }}</td>
                <td style="color: #b45309;">{{ number_format($sem->ofrenda_total * 0.40, 2) }}</td>
                <td style="color: #0891b2;">{{ number_format($sem->ofrenda_total * 0.60, 2) }}</td>
                <td>{{ number_format($sem->pro_templo_total, 2) }}</td>
                <td class="bg-light fw-bold">Bs {{ number_format($subTotalSemana, 2) }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot style="background-color: #f1f5f9; font-weight: bold;">
            <tr>
                ...
                <td>{{ number_format($ofrendaMision40, 2) }}</td>
                <td>{{ number_format($ofrendaLocal60, 2) }}</td>
